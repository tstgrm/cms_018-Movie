...
<a href="<?php the_permalink(); ?>" class="c-post-case">
	<figure class="c-figure c-post-case__figure">
		<?php
		if ( has_post_thumbnail() ) {
			the_post_thumbnail(
				'post-thumbnail',
				array(
					'class' => 'c-post-case__img c-figure__img',
					'alt'   => wp_strip_all_tags( get_the_title() ),
				)
			);
		} else {
			echo '<img src="' . esc_url( get_template_directory_uri() ) . '/_static/dist/img/noimage.png" alt="noimage" class="c-post-case__img c-figure__img">';
		}
		?>
	</figure>
	<?php
	$qr = '';
	if ( get_field( 'qr' ) ) {
		$qr = ' c-post-case__content--qr';
	}
	?>
	<div class="c-post-case__content<?php	echo esc_attr( $qr ); ?>">
		<?php if ( $qr ) : ?>
			<img src="<?php the_field( 'qr' ); ?>" alt="QRコード" class="c-post-case__qr">
		<?php endif; ?>
		<p class="c-post-case__ttl"><?php the_title(); ?></p>
			...
			while ( have_rows( 'outline' ) ) :
				...
				?>
					<?php if ( get_sub_field( 'name' ) ) : ?>
						<p class="c-post-case__company"><?php the_sub_field( 'name' ); ?></p>
					<?php endif; ?>
			<?php endwhile; ?>
		<?php endif; ?>

		<div class="c-post-case__meta">
			<div class="c-tags">
				<?php
